...
my( $opt, $usage ) = describe_options(
  ...
  [ 'cell=i', 'cell id to play', { required => 1 } ],
...
sub run {
  ...
  my $url = sprintf "http://%s:%d/games/%d/play/%d", 
                    $opt->host, $opt->port, $opt->game, $opt->cell;
